...
const RestaurantDetailScreen = () => {
  ...
  // ✅ Added getLiveAvailableSeats to destructure
  const { selectedRestaurant, getLiveAvailableSeats } = useBooking();
  const r = selectedRestaurant;

  // ✅ liveSeats state — starts at 0 while loading, gets filled by useFocusEffect
  const [liveSeats, setLiveSeats] = useState<number>(0);

  // ✅ useFocusEffect so this re-fetches every time user navigates back to this screen
  // (e.g. after making a booking or cancelling one)
  useFocusEffect(
    useCallback(() => {
      if (r) {
        getLiveAvailableSeats(r.id).then(setLiveSeats);
      }
    }, [r?.id])
  );
  ...
  // ✅ All derived values now use liveSeats instead of r.availableSeats
  const pct = Math.round((liveSeats / r.totalSeats) * 100);
  const seatColor = pct <= 20 ? Colors.danger : pct <= 50 ? Colors.warning : Colors.success;
  const statusMsg =
    pct === 0 ? 'Fully Booked' :
    pct <= 20 ? 'Almost Full — Book soon' :
    pct <= 50 ? 'Filling up fast' :
    'Good availability';
    ...
    <View style={styles.container}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.seatCard}>
            ...
            <View style={styles.seatNumbers}>
              {/* ✅ Fix 1: Available — was r.availableSeats */}
              <View style={styles.seatStat}>
                <Text style={[styles.seatNum, { color: seatColor }]}>{liveSeats}</Text>
                <Text style={styles.seatNumLabel}>Available</Text>
              </View>
              <View style={styles.seatStatDivider} />
              {/* ✅ Fix 2: Occupied — was r.totalSeats - r.availableSeats */}
              <View style={styles.seatStat}>
                <Text style={styles.seatNum}>{r.totalSeats - liveSeats}</Text>
            ...
            {/* ✅ Fix 3: Progress bar width — pct is now derived from liveSeats */}
      ...
      <View style={styles.bottomBar}>
        <View style={styles.bottomLeft}>
          ...
          {/* ✅ Fix 4: Bottom bar text — was r.availableSeats */}
          <Text style={[styles.bottomValue, { color: seatColor }]}>{liveSeats} seats free</Text>
        </View>
        <TouchableOpacity
          // ✅ Fix 5: Disabled state — was r.availableSeats === 0
          style={[styles.bookBtn, liveSeats === 0 && styles.bookBtnDisabled]}
          onPress={() => navigation.navigate('Booking')}
          disabled={liveSeats === 0}
          activeOpacity={0.85}
        >
          <Text style={styles.bookBtnText}>
            {liveSeats === 0 ? 'FULLY BOOKED' : 'RESERVE A TABLE'}
          </Text>
        </TouchableOpacity>
      </View>
    </View>
